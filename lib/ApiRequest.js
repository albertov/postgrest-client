...
class ApiRequest {
  ...
  /**
   * Instructs the server to return a content-range header so fullLength
   * works
   *
   * @param {string|void} Mode (default: 'exact')
   * @returns {ApiRequest} The API request object.
   */

  withCount (mode = 'exact') {
    return this.set('prefer', 'count='+mode)
  }
}
